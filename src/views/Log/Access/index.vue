  ...
  <div>
    ...
    >
      ...
      <template #responseTime="slotProps">
        <a-tag color="purple">{{ slotProps.responseTime - slotProps.requestTime }} ms</a-tag>
      </template>
  ...
  <a-modal :width="1100" v-model:visible="visible" :title="$t('Access.index.480752-0')">
    ...
    >
      ...
      <a-descriptions-item :label="$t('Access.index.480752-5')">
        {{ dayjs(descriptionsData?.requestTime).format('YYYY-MM-DD HH:mm:ss') }}
      </a-descriptions-item>
      <a-descriptions-item :label="$t('Access.index.480752-6')">
        {{ descriptionsData?.responseTime - descriptionsData?.requestTime + 'ms' }}
      </a-descriptions-item>
