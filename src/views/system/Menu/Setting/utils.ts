...
import type { AntTreeNodeDropEvent, TreeProps, TreeDataItem } from 'ant-design-vue/es/tree';
  ...
  const mergeItem: any = (oldItem: any, newItem: any) => {
    ...
    if (oldItem.children && newItem.children) {
      ...
      for (const oldChild of oldItem.children) {
        const index = newChildren.findIndex(child => child.code === oldChild.code);
        if (index !== -1) {
  ...
  for (const oldItem of oldData) {
    if (!filterCodeSet.has(oldItem.code)) {
      ...
      if (!newItem) {
        ...
        if (!filterCodeSet.has(oldItem.code)) {
          if (oldItem.children) {
            oldItem.children = oldItem.children.filter((i: any) => !filterCodeSet.has(i.code));
          }
        }
...
export const select = (selecteds: Array<string>, e: any, treeData: any) => {
  ...
  const { parents, children } = findAllParentsAndChildren(treeData, node.code);
  const Keys = new Set(selecteds);
  const selectedAllKeys = selected ? [...[...parents, ...children]] : [...children];
