<template>
  <j-page-container>
    <a-card>
      ...
      <a-button type="primary" @click="() => (visible = true)" style="margin-left: 10%">
...
import { getSystemPermission as getSystemPermission_api, updateMenus } from '@/api/initHome';
import {
  filterMenu,
  inItSelected,
  drop,
...
const onDragend = (info: AntTreeNodeDropEvent) => {
  const { node } = info;
  const { children } = findAllParentsAndChildren(cloneDeep(treeData.value), node.code);
