<template>
  <div class="calendarRight">
    <div class="tips">
      <span>{{ $t('CalendarRight.index.810845-0') }}</span>
      <j-permission-button
        type="text"
      ...
      >
        ...
        {{ $t('CalendarRight.index.810845-2') }}
      </j-permission-button>
    </div>
    <FullCalendar
      ref="Calendar"
    ...
    <div class="rapidAction">
      <div class="rapidSwitch">
        <a-tooltip>
          <template #title>{{ $t('CalendarRight.index.810845-3') }}</template>
          <AIcon type="QuestionCircleOutlined"></AIcon>
        </a-tooltip>
        {{ $t('CalendarRight.index.810845-4') }}
        <a-switch v-model:checked="rapidChecked" />
      </div>
      <div class="rapidActionControl" v-if="rapidChecked">
        <a-space>
          ...
          <a-input style="width: 300px" v-model:value="selectedDate" :disabled="true"></a-input>
        </a-space>
        <a-space>
          <j-ellipsis>
        ...
        <div>
          <a-button type="text" @click="reselect">
            {{ $t('CalendarRight.index.810845-8') }}
          </a-button>
