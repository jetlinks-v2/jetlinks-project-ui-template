...
const handleViewDidMount = async arg => {
  ...
  const endDate = dayjs(arg.view.activeEnd).subtract(1, 'day').format('YYYY-MM-DD');
...
const eventChange = computed(() => {
  return JSON.stringify(eventsData.value) !== JSON.stringify(initialData.value);
});
// 自定义切换月份逻辑
...
const handleDateClick = selectInfo => {
  ...
  if (!choiceStart.value) {
    ...
    const selectedDate = document.querySelector(`.fc-day[data-date="${selectInfo.dateStr}"]`);
  ...
  info.el.addEventListener('mouseenter', () => {
    ...
    if (lastHoverDate.value && !choiceEnd.value) {
      lastHoverDate.value.forEach(i => {
        const selectedDate = document.querySelector(`.fc-day[data-date="${i}"]`);
        selectedDate.classList.remove('selectedDate');
      });
    }
    if (choiceStart.value && currentDate.isAfter(choiceStart.value) && !choiceEnd.value) {
      const dateArr = getDatesBetween(choiceStart.value, currentDate.add(1, 'day'));
      dateArr.forEach(i => {
        const selectedDate = document.querySelector(`.fc-day[data-date="${i}"]`);
        selectedDate.classList.add('selectedDate');
      });
    ...
    if (effectData[i].length) {
      effectData[i].forEach(item => {
        item.date = dayjs(item.date)
          .add(selectData.length * Math.ceil((i + 1) / selectData.length), 'day')
          .format('YYYY-MM-DD');
      });
    } else {
