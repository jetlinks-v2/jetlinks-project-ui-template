<template>
  <div class="api-does-container">
    ...
    <p>
      ...
      <span>{{ getContent(selectApi.requestBody) || 'application/x-www-form-urlencoded' }}</span>
    ...
    <div class="api-card" v-if="requestCard.codeText !== undefined">
      ...
      <Monaco :tips="requestCard.tips" :codeText="requestCard.codeText" :loading="loading" />
    </div>
      ...
      <div class="content">
        ...
        >
          <template #required="slotProps">
            <span :style="{ color: Boolean(slotProps.required) ? '#f81d22' : '' }">
    ...
    <div class="api-card">
      ...
      <div class="content">
        ...
        <a-tabs v-model:activeKey="responseStatusCard.activeKey">
          <a-tab-pane :key="key" :tab="key" v-for="key in tabs"></a-tab-pane>
        </a-tabs>
      </div>
    </div>
    ...
    <div class="api-card">
      ...
      <Monaco :tips="respParamsCard.tips" :codeText="respParamsCard.codeText" :loading="loading" />
    </div>
  </div>
</template>
    ...
    if (!props.selectApi.requestBody) {
      requestCard.tableData = props.selectApi.parameters?.map(i => {
        const desc = i.description
          ? i.description +
            (i.schema?.enum?.length
              ? `,${$t('components.ApiDoes.5497514-12')}:${i.schema?.enum.join(',')}`
              : '')
          : '';
        const _paramsType = i.schema?.type
          ? i.schema?.type + (i.schema?.format ? `(${i.schema?.format})` : '')
          : '';
        return {
          ...i,
          paramsName: i.name,
...
const responseStatusCard = reactive<cardType>({
  ...
  tableData: [],
  getData: () => {
    if (!Object.keys(props.selectApi.responses).length) return (responseStatusCard.tableData = []);
...
const respParamsCard = reactive<cardType>({
  ...
  codeText: '',
  getData: (code: string) => {
    const schemaName = responseStatusCard.tableData.find((item: any) => item.code === code)?.schema;
