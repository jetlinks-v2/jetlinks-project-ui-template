...
const getTreeData = () => {
  ...
  getTreeOne_api().then((resp: any) => {
    ...
    else if (props.mode === 'home') allPromise.push(getApiGranted_api(props.code as string));
      ...
      .then(values => {
        values.forEach((item: any, i) => {
          if (props.mode === 'api') {
            ...
            const paths = filterPath(item.paths, values[values.length - 1].result as string[]);
...
const combData = (dataSource: object) => {
  ...
  keys.forEach(key => {
    ...
    let apiObj: treeNodeTpye | undefined = apiList.find(item => item.name === name);
    if (apiObj) {
      apiObj.apiList?.push({
        url: key,
    ...
    if (Object.prototype.hasOwnProperty.call(path, key)) {
      ...
      for (const prop in value) {
        if (Object.prototype.hasOwnProperty.call(value, prop)) {
          ...
          if (!filterArr.includes(item.operationId)) delete value[prop];
        }
      }
