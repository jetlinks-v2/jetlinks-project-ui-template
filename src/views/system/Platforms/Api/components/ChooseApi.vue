...
import { addOperations_api, delOperations_api, updateOperations_api } from '@/api/system/apiPage';
import type { modeType } from '../typing';
...
const rowSelection = {
  ...
  onChange: (keys: string[], _data: any[]) => {
    ...
    // 当前表格, 原有选中的id
    // const oldSelectedKeys = currenTableKeys.filter(key => props.sourceKeys.includes(key));
    // 除当前表格之外, 勾选上的数据
  ...
  if (props.mode === 'api') {
    ...
    if (addKeys.length || removeKeys.length) {
      if (removeKeys.length) {
        await delOperations_api(removeKeys);
      }
    ...
    const items = props.selectedRowKeys.map(key => ({
      id: key,
      permissions: department.changedApis[key]?.security
        ? department.changedApis[key]?.security
        : [],
    }));
