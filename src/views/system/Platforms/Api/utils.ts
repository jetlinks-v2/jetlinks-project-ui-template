  ...
  arr.forEach(item => {
    ...
    switch (item.paramsType) {
      ...
      case 'array':
        ...
        break;
      case 'object': {
        const _itemType = schemas.items?.type;
      ...
      default: {
        if (item.children) {
          ...
          const _result = getCodeText(schemas, item.children, level - 1, paths);
          ...
          const newArr = Object.entries(properties).map((item: [string, any]) => {
            const [paramsName, extra] = item;
          ...
          const _result = getCodeText(schemas, newArr, level - 1, paths);
          value = _result.codeText;
          tips.push(..._result.codeTips);
          tips.push(undefined);
        }
