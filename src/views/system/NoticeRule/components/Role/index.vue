<template>
  <div class="role">
    <template v-if="type !== 'add'">
      <a-input-search allowClear @search="onSearch" :placeholder="$t('Role.index.617700-0')" />
      <div class="role-alert">
        <a-alert type="info">
          <template #message>
            <div class="header">
              <a-checkbox :indeterminate="indeterminate" :checked="checked" @change="onSelectAll">
                {{ $t('Role.index.617700-1') }}
              </a-checkbox>
              <a-space v-if="_selectedRowKeys.length">
                <span>
                  {{ $t('Role.index.617700-2') }}{{ _selectedRowKeys.length
                  }}{{ $t('Role.index.617700-3') }}
                </span>
                <a-button style="padding: 0; height: 22px" type="link" @click="cancelSelect">
                  {{ $t('Role.index.617700-4') }}
                </a-button>
              </a-space>
            </div>
          </template>
