        ...
        <a-step v-for="(item, index) in stepList" :key="item">
          <template #title>
            ...
            <a-tooltip v-if="index === 4">
              <template #title>
                <span>
                  {{ $t('Save.index.320308-1', [name, showName]) }}
                  <br />
        ...
        <div>
          ...
          <div style="margin: 50px 200px">
            <a-form ref="formRef" :model="formModel" layout="vertical">
              ...
              >
                <a-input v-model:value="formModel.name" :placeholder="$t('Save.index.320308-14')" />
              </a-form-item>
            </a-form>
          </div>
        </div>
    ...
    <template #footer>
      <a-space>
        <a-button v-if="current === 0" @click="emit('close')">
          {{ $t('Save.index.320308-16') }}
        </a-button>
        <a-button v-else @click="onPrev">{{ $t('Save.index.320308-17') }}</a-button>
        <a-button type="primary" @click="onNext" v-if="current !== stepList.length - 1">
          {{ $t('Save.index.320308-18') }}
        </a-button>
        <a-button :loading="loading" type="primary" @click="onSave" v-else>
          {{ $t('Save.index.320308-19') }}
        </a-button>
      </a-space>
    </template>
  </a-modal>
