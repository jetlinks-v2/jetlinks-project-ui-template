<template>
  <div class="header">
    <pro-search
      ...
      @search="handleSearch"
    />
    <j-permission-button @click="onAdd" type="primary" :hasPermission="['notice/Config:add']">
    ...
    >
      <template #card="slotProps">
        ...
        >
          <template #img>
            <slot name="img">
              <img :src="getLogo(slotProps.type, slotProps.provider)" class="notify-logo" />
            </slot>
          </template>
          <template #content>
            ...
            <a-row>
              ...
              <a-col :span="12">
                <div class="card-item-content-text">
                  {{ $t('components.NotifyConfig.517040-3') }}
                </div>
                <j-ellipsis>
...
const onAdd = () => {
  const tab: any = window.open(
    `${origin}/#/iot/notice/Config/detail/:id?notifyType=${noticeType.get(props.notifyType)}`,
  );
