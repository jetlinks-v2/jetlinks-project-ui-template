<template>
  <div style="padding: 50px 200px">
    <a-form v-if="variableDefinitions.length" :layout="'vertical'" ref="formRef" :model="modelRef">
      <a-form-item
        :name="`${item?.id}`"
        :label="item?.name"
      ...
      >
        <Tag :notify="notify" v-if="getType(item) === 'tag'" v-model:value="modelRef[item.id]" />
        <InputFile v-else-if="getType(item) === 'file'" v-model:value="modelRef[item.id]" />
        <a-input v-else-if="getType(item) === 'link'" v-model:value="modelRef[item.id]" />
        <BuildIn
          v-else
          :item="item"
    ...
    <j-empty
      v-else
      style="margin: 20px 0"
      :description="$t('components.VariableDefinitions.420465-0')"
    />
  </div>
</template>
      ...
      if (value?.source === 'upper') {
        if (!value?.upperKey) {
          return Promise.reject(
            new Error($t('components.VariableDefinitions.420465-3') + item.name),
          );
        } else {
          return Promise.resolve();
        }
