...
const handleSearch = async () => {
  ...
  if (props.data?.channelConfiguration?.notifierId) {
    const resp = await ConfigApi.detail(props.data?.channelConfiguration?.notifierId);
    if (resp.status === 200) {
  ...
  if (props.data?.channelConfiguration?.templateId) {
    const resp = await TemplateApi.detail(props.data?.channelConfiguration?.templateId);
    if (resp.status === 200) {
      obj.template = resp.result?.name;
      obj.content = resp.result?.template?.message;
      if (resp.result?.variableDefinitions?.length && props.data?.channelConfiguration?.variables) {
        const config = props.data?.channelConfiguration?.variables;
        const t = Object.keys(config)?.find((i: any) => config?.[i]?.source === 'upper');
        if (t && !builtInList.value.length) {
          const _variables = await queryConfigVariables(props.data.providerId);
          if (_variables.status === 200) {
            // 避免数据id相同，去重
            const _set = new Set((_variables.result as any[]).map(item => item?.id));
        ...
        obj.variables = (resp.result?.variableDefinitions || []).map((item: any) => {
          const _item = config?.[item?.id];
          return {
            name: item.name,
            value:
              _item?.value ||
              builtInList.value.find(i => _item.upperKey === i.id)?.name ||
              _item.upperKey,
          };
        });
      }
