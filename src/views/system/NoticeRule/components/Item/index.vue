    ...
    <div class="child-item">
      <div class="child-item-left">
        ...
        <div>
          <a-tooltip :title="!action ? $t('Item.index.538002-0') : ''">
            <a-switch :disabled="!action" @change="onSwitchChange" :checked="checked" />
          </a-tooltip>
        </div>
        <div class="child-item-left-auth" :class="{ disabled: !checked }">
          <a-tooltip>
            ...
            <a-button :disabled="!update || !checked" type="text" @click="onAuth">
              <span v-if="!auth.length" class="child-item-left-auth-lock">
                <AIcon type="LockFilled" />
              </span>
              <span v-else class="child-item-left-auth-key"><AIcon type="KeyOutlined" /></span>
              <span class="child-item-left-auth-text">{{ $t('Item.index.538002-3') }}</span>
            </a-button>
          </a-tooltip>
        </div>
      </div>
      <div class="child-item-right" :class="{ disabled: !checked }">
            ...
            <div class="box-item">
              <a-dropdown>
                <div class="box-item-img">
                  <img style="width: 100%" :src="iconMap.get(slotProps?.channelProvider)" />
                </div>
                  ...
                  <a-menu mode="">
                    ...
                    <a-menu-item>
                      <j-permission-button
                        ...
                        type="link"
                        :hasPermission="['system/NoticeRule:update']"
                      >
                    ...
                    <a-menu-item>
                      <j-permission-button
                        ...
                        danger
                        type="link"
                        :hasPermission="['system/NoticeRule:delete']"
                      >
        ...
        <a-tooltip :title="!add ? $t('Item.index.538002-0') : ''">
          <a-button class="box-item-add" :disabled="!add" @click="onAdd">
...
const onAction = (e: boolean) => {
  if (e) {
    ...
    if (
      ...
      props.data.channels.find((item: any) => item.channelProvider === 'inside-mail')
    ) {
      spinning.value = true;
...
const onSwitchChange = (e: boolean) => {
  ...
  if (_checked) {
    ...
    if (e) {
      ...
      Modal.confirm({
        title: e ? $t('Item.index.538002-9') : $t('Item.index.538002-10'),
