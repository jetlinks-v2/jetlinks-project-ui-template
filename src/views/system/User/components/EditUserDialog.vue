  ...
  >
    <a-form ref="formRef" :model="form.data" layout="vertical">
      <div class="formName" v-if="form.IsShow('add', 'edit')">
        {{ $t('components.EditUserDialog.939453-2') }}
      </div>
            ...
            :rules="[
              {
                ...
                message: $t('components.EditUserDialog.939453-13'),
              },
            ]"
          >
            <form-item-role
      ...
      <div class="formName" v-if="form.IsShow('add', 'edit')">
        {{ $t('components.EditUserDialog.939453-16') }}
      </div>
      <a-row :gutter="24" v-if="form.IsShow('add', 'edit')">
        <a-col :span="24">
          <a-form-item
  ...
  rules: {
    ...
    checkPassword: (_rule: Rule, value: string): Promise<any> =>
      new Promise((resolve, reject) => {
        ...
        else if (!passwordRegEx(value)) return reject($t('components.EditUserDialog.939453-28'));
...
const checkCh = async (_rule: Rule, value: string) => {
  if (/[\u4e00-\u9fa5]/.test(value))
    return Promise.reject($t('components.EditUserDialog.939453-30'));
