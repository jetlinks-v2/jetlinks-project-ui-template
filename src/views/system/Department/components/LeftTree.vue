  ...
  getTreeData_api(params)
    .then((resp: any) => {
      ...
      sourceTree.value = resp.result.sort((a: any, b: any) =>
        a.sortIndex === b.sortIndex ? b.createTime - a.createTime : a.sortIndex - b.sortIndex,
      ); // 报存源数据
...
const openDialog = (row: any = {}) => {
  ...
  if (!row.id) {
    ...
    const _index = children?.length > 0 ? children?.map(item => item.sortIndex) : [0];
