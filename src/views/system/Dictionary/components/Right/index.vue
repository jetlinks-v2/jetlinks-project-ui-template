<template>
  <div class="des">
    <div class="des_head">
      <div>
        {{ $t('Right.index.572408-0') }}
        <span>{{ data.id }}</span>
      </div>
      <div style="display: flex">
        {{ $t('Right.index.572408-1') }}
        <j-ellipsis style="width: calc(100% - 200px)">
          <span>{{ data.describe }}</span>
        </j-ellipsis>
      </div>
      <div>
        {{ $t('Right.index.572408-2') }}
        <span v-time-format="'YYYY-MM-DD HH:mm:ss'">{{ data?.createTime }}</span>
      </div>
    </div>
    <div class="contain">
      <pro-search
...
const getActions = (data: Partial<Record<string, any>>, type: 'card' | 'table'): any[] => {
  if (!data) return [];
  const actions = [
    {
      key: 'update',
