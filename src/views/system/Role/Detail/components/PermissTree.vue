          ...
          <div v-else-if="record.accessSupport.value === 'support'">
            ...
            >
              <j-radio
                ...
                v-for="(asset, _i) in record.assetAccesses"
                :key="_i"
                >{{ asset.name }}</j-radio
              >
            </j-radio-group>
...
import { getPrimissTree_api } from '@/api/role'
...
function getAllPermission() {
  ...
  getPrimissTree_api(id).then((resp) => {
    ...
    // 默认选中个人中心相关设置
    tableData.value = _result
    // .map(
    //   (item: { code: string; buttons: any[]; granted: boolean }) => {
  ...
  () => {
    ...
    const selected = cloneDeep(flatTableData).filter(
      (item: any) =>
        ...
        // item.code === USER_CENTER_MENU_CODE ||
        // item.code === MESSAGE_SUBSCRIBE_MENU_CODE, // 放开个人中心以及消息订阅
    )
