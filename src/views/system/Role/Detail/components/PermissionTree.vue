    ...
    >
      ...
      <template #headerCell="{ column }">
        ...
        <div v-else-if="column.key === 'data'">
          ...
          <a-checkbox v-model:checked="bulkShow" @change="bulkValue = ''" style="margin-left: 10px">
      ...
      <template #bodyCell="{ column, record }">
        ...
        <div v-else-if="column.key === 'data'">
          ...
          <div v-else-if="record.accessSupport.value === 'support'">
            <a-radio-group v-model:value="record.selectAccesses" @change="resetBulk">
              <a-radio
    ...
    () => {
      ...
      const selected = cloneDeep(flatTableData).filter(
        (item: any) =>
          ...
          (item.indeterminate && item.buttons) || item.granted, // 放开个人中心
      );
 ...
 */
function menuChange(row: tableItemType, setButtonBool: boolean = true): undefined {
  ...
  if (setButtonBool) {
    ...
    if (row.children) {
      setChildrenChecked(row.children, row.granted);
    }
  }
  // 更新选中状态
  ...
  if (row.parentId) {
    ...
    const parent = flatTableData.find(item => item.id === row.parentId) as tableItemType;
...
const updateAuthority = (row: any) => {
  if (row.accessSupport && row.accessSupport.value === 'support') {
    // 如果当前数据权限已有值，且菜单权限没有被选中或被半选   则清空对应的数据权限
    if (row.selectAccesses && !row.granted && !row.indeterminate) row.selectAccesses = '';
    // 如果当前数据权限没有值，且菜单权限有被选中或者是被半选   则将数据权限变为默认值'creator'
    else if (!row.selectAccesses && (row.granted || row.indeterminate))
      row.selectAccesses = 'creator';
  }
  if (row.children?.length > 0) {
    row.children?.forEach((item: any) => {
      if (item.accessSupport && item.accessSupport.value === 'support') {
        // 如果当前数据权限已有值，且菜单权限没有被选中或被半选   则清空对应的数据权限
        if (item.selectAccesses && !item.granted && !item.indeterminate) item.selectAccesses = '';
        // 如果当前数据权限没有值，且菜单权限有被选中或者是被半选   则将数据权限变为默认值'creator'
        else if (!item.selectAccesses && (item.granted || item.indeterminate))
          item.selectAccesses = 'creator';
      }
      if (item.children) {
...
function setChildrenChecked(_children: tableItemType[], value: boolean) {
  ...
  _children.forEach(item => {
    ...
    if (item.children) {
      setChildrenChecked(item.children, value);
    }
  });
}
/**
 ...
 * @param prop 目标属性
 */
function setStatus(target: tableItemType, prop: 'children' | 'buttons' = 'children') {
  ...
  if (childrens && childrens instanceof Array) {
    ...
    const indeterminateLen = childrens.filter(
      (childrens: buttonItemType | tableItemType) => childrens?.indeterminate,
    ).length;
