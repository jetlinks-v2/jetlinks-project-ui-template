  ...
  >
    <div>
      <div style="background-color: #f8f9fc; padding: 24px">
        <a-steps :current="current" size="small" progress-dot @change="onChange">
          <a-step :title="item" v-for="(item, index) in list" :key="item">
    ...
    <template #footer>
      <a-button v-if="current === 0" @click="emits('close')">
        {{ $t('EditPassword.index.010161-10') }}
      </a-button>
      <a-button v-if="current === 2" @click="onPrev">
        {{ $t('EditPassword.index.010161-11') }}
      </a-button>
      <a-button type="primary" v-else @click="onNext">
        {{ $t('EditPassword.index.010161-12') }}
      </a-button>
      <a-button v-if="current === 2" type="primary" @click="handleOk">
        {{ $t('EditPassword.index.010161-13') }}
      </a-button>
    </template>
  </a-modal>
</template>
...
import { updateMepsd_api, checkOldPassword_api, validateField_api } from '@/api/account/center';
...
const list = [
  $t('EditPassword.index.010161-14'),
  $t('EditPassword.index.010161-15'),
  $t('EditPassword.index.010161-16'),
];
...
const checkMethods = {
  old: async (_rule: any, value: string) => {
    ...
    try {
      ...
      if (resp.status === 200 && !resp.result.passed) return Promise.reject(resp.result.reason);
