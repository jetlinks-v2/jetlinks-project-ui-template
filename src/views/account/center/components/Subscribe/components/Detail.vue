<template>
    <div style="width: 300px">
        ...
        <template v-else-if="getType === 'notifier-email'">
            <div class="tip"><j-ellipsis :lineClamp="2">绑定账号：{{ user.userInfo?.email }}</j-ellipsis></div>
        </template>
        <template v-else>
            <div class="tip"><j-ellipsis :lineClamp="2">绑定账号：{{ user.userInfo?.telephone }}</j-ellipsis></div>
        </template>
        <div class="btn">
