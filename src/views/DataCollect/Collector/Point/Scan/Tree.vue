    ...
    <a-spin :spinning="spinning">
      ...
      >
        <template #title="{ name, key }">
          <span
            :class="[selectKeys.includes(key) ? 'tree-selected' : 'tree-title']"
          >
            {{ name }}
          </span>
        </template>
...
import { scanOpcUAList, queryPointNoPaging } from '@/api/data-collect/collector'
...
const handleData = (arr: any): any[] => {
  const data = arr.filter((item: any) => {
    return (isSelected && !selectKeys.value.includes(item.id)) || !isSelected
  })
...
watch(
  () => isSelected.value,
  (value) => {
    treeData.value = value ? handleData(treeAllData.value) : treeAllData.value
  },
  { deep: true },
)
watch(
