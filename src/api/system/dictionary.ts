...
export const verifyId = (id: string) =>
  request.post(`/dictionary/_exists`, { where: `id is ${id}` });
 ...
 * 查询字典项不分页
 */
export const queryDicItemNoPage = (data: any) =>
  request.post('/dictionary-item/_query/no-paging', data);
 ...
 * 更新字典项
 */
export const updateDicItem = (id: string, data: any) => request.put(`/dictionary-item/${id}`, data);
