...
type FindComponentsFn = (
  ...
  meta: any,
  hasChildren: false,
) => undefined | VNode | ShallowRef | FnType;
...
export const handleMenus = (menuData: any, extraMenus: any, components: any, level: number = 1) => {
  if (menuData && menuData.length) {
    return menuData
      .filter(item => !filterMenuCode.includes(item.code))
      .map(item => {
        ...
        route.component =
          item.component ??
          findComponents(item.code, level, isApp, components, meta, route.children.length);
