...
<template>
  ...
  <div class="dash-board" :style="{ backgroundColor: timeLineConfig?.componentProps?.background }">
    <div class="header">
    ...
    <div>
      <j-empty v-if="isEmpty" style="height: 200px; margin-top: 100px" />
      <template v-else>
    ...
    if (res.result?.length) {
      ...
      filterArray.forEach((item: any) => {
        ...
        if (props.timeLineConfig.type === 'Cpu') {
          ...
          _value = (((memoryJvmHeapTotal - memoryJvmHeapFree) / memoryJvmHeapTotal) * 100).toFixed(
            2,
          );
        }
