<template>
  <!-- 设备消息、点位数据量Echarts -->
  <div class="message-card" :style="{ backgroundColor: data.componentProps.background }">
    ...
    <div class="message-chart" style="width: 100%">
    ...
    if (Object.keys(paramsData.value)?.length) {
      ...
      trendData_api([_paramsData]).then((res: any) => {
        if (res.success) {
          let x = res.result
            .map((item: any) =>
              _time === '1h' ? `${item.data.timeString}时` : item.data.timeString,
            )
            .reverse();
          ...
          trendOptions.value = setChartOption(
            x,
            y,
            maxY,
            props.data.componentProps.name,
            '#82e5d2',
          );
        }
      });
