  ...
  <div class="live-player-warp">
    <div class="live-player-content">
      ...
      <div class="player-screen-tool" v-if="showScreen">
        <a-radio-group :value="screen" button-style="solid" @change="handleScreenChange">
        ...
        <div class="screen-tool-save">
          <a-space>
            ...
            <a-popover v-model:visible="visible" trigger="click" title="分屏名称">
              <template #content>
              ...
              <a-dropdown-button type="primary" @click="visible = true">
                保存
                <template #overlay>
                  <a-menu>
                    <j-empty v-if="!historyList.length" description="暂无数据" />
                    <a-menu-item
      ...
      <div class="player-body">
        <div ref="fullscreenRef" class="player-screen" :class="`screen-${screen}`">
          <template v-for="(item, index) in players" :key="item.key">
            <div
              class="player-screen-item"
              :class="{
                active: showScreen && playerActive === index && !isFullscreen,
 ...
 * @param index
 */
const reloadPlayer = (id: string, channelId: string, url: string, index: number) => {
